...
class Consulta:
    """
    Classe Consulta
    Armazena os dados da expressão de busca:
...
class Resposta:
    ...
    # quantidade de documentos max esperados por página
    qtd_docs_por_pagina = 50

    def __init__(self, conteudo):
        # conteudo da página pura
    ...
    def dados(self):
        if not self._dados:
            try:
                ...
                logging.exception(
                    f"Resultado da consulta '{self.conteudo}': tipo inválido."
                )
            except json.JSONDecodeError:
                logging.exception(
                    f"Resultado da consulta '{self.conteudo}': JSON inválido."
                )
            else:
                self._dados = j
        return self._dados
    ...
    @property
    def documentos(self):
        # documentos retornados na página
